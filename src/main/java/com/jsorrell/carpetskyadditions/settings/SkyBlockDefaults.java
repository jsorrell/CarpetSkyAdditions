...
public final class SkyBlockDefaults {
  public static void writeDefaults(Path rootSavePath) throws IOException {
    ...
    writeSkyBlockDefaults(rootSavePath.resolve(Build.MODID + ".conf"), defaultsPath.resolve("default_" + Build.MODID + ".conf"));
  ...
  private static void writeCarpetDefaults(Path configPath, Path defaultConfigPath) throws IOException {
    writeConfigFile(configPath, defaultConfigPath, List.of(
      new FieldPair("renewableSponges", "true"),
      new FieldPair("piglinsSpawningInBastions", "true")
    ));
  }
